...
OSF.OUtil=(function () {
	...
	return {
		...
		parseXdmInfoWithGivenFragment: function OSF_OUtil$parseXdmInfoWithGivenFragment(skipSessionStorage, fragment) {
			return OSF.OUtil.parseInfoWithGivenFragment(_xdmInfoKey, _xdmSessionKeyPrefix, _serializerVersionKey, skipSessionStorage, fragment);
		},
		...
		parseSerializerVersionWithGivenFragment: function OSF_OUtil$parseSerializerVersionWithGivenFragment(skipSessionStorage, fragment) {
			return parseInt(OSF.OUtil.parseInfoWithGivenFragment(_serializerVersionKey, _serializerVersionKeyPrefix, _appContextKey, skipSessionStorage, fragment));
		},
		parseInfoWithGivenFragment: function OSF_OUtil$parseInfoWithGivenFragment(infoKey, infoKeyPrefix, followInfoKey, skipSessionStorage, fragment) {
			var fragmentParts=fragment.split(infoKey);
			var infoValueWithFollowPart=fragmentParts.length > 1 ? fragmentParts[fragmentParts.length - 1] : null;
			var infoValue=infoValueWithFollowPart !=null ? (infoValueWithFollowPart.split(followInfoKey))[0] : null;
...
(function (OfficeExt) {
	...
	(function (WACUtils) {
		...
		function addAppContextAsHash(url, appContext) {
			return OSF.OUtil.addInfoAsHash(url, _appContextKey, appContext);
		}
		WACUtils.addAppContextAsHash=addAppContextAsHash;
		function parseAppContextWithGivenFragment(skipSessionStorage, fragment) {
			return OSF.OUtil.parseInfoWithGivenFragment(_appContextKey, _appContextKeyPrefix, null, skipSessionStorage, fragment);
		}
